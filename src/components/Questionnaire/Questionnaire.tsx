...
export default function Questionnaire({ onComplete }: Props) {
  ...
  const next = () => {
    ...
    if (step === 2 && !responses.thermalComfort) {
      setError("Please rate your thermal comfort");
      return;
    }
    if (
      step === 3 &&
      (!responses.surroundings || responses.surroundings.length === 0)
    ) {
      setError("Please select at least one option");
      return;
    }
    if (step === 4 && !responses.naturalLight) {
      setError("Please rate the natural light");
      return;
    }
    if (step === 5 && !responses.noiseLevel) {
      setError("Please select a noise level");
      return;
    }
    if (step === 6 && !responses.activity) {
      setError("Please select your current activity");
      return;
    }
    if (step === 7 && !responses.shelter) {
      setError("Please select a shelter option");
      return;
    }
    if (step === 8 && !responses.affordanceRating) {
      setError("Please rate how well this environment supports your work");
      return;
    }
      ...
      <div style={S.body}>
        ...
        {step === 2 && (
          <>
            ...
            <p style={S.question}>
              How comfortable is the temperature right now?
            </p>
        ...
        {step === 3 && (
          <>
            ...
            <p style={S.pOKW2hint}>
              pOKW2: environment type — select all that apply
            </p>
            <div style={S.optGrid}>
              {[
                ...
                <button
                  ...
                  style={optStyle(
                    responses.surroundings?.includes(opt.val) ?? false,
                  )}
                >
                  <span style={iconStyle}>{opt.icon}</span>
                  {opt.label}
                ...
                [
                  {
                    ...
                    icon: "🔇",
                    label: "Quiet",
                    sub: "Little to no background noise",
                  },
                  {
                    val: "moderate",
                    icon: "🔉",
                    label: "Moderate",
                    sub: "Some background noise, not distracting",
                  },
                  {
                    val: "loud",
                    icon: "🔊",
                    label: "Loud / distracting",
                    sub: "Hard to concentrate",
                  },
                ] as const
              ).map((opt) => (
                <button
                  ...
                  <span>
                    ...
                    <span
                      style={{
                        fontSize: "11px",
                        color:
                          responses.noiseLevel === opt.val ? "#3a5a2a" : "#aaa",
                      }}
                    >
                      {opt.sub}
                    </span>
        ...
        {step === 7 && (
          <>
            ...
            <p style={S.pOKW2hint}>
              pOKW2: semi-outdoor classification — shelter indicator
            </p>
            <div style={{ ...S.optGrid, gridTemplateColumns: "1fr" }}>
              {(
                [
                  ...
                  <span>
                    ...
                    <span
                      style={{
                        fontSize: "11px",
                        color:
                          responses.shelter === opt.val ? "#3a5a2a" : "#aaa",
                      }}
                    >
                      {opt.sub}
                    </span>
        ...
        {step === 8 && (
          <>
            ...
            <p style={S.question}>
              How well does this environment support your knowledge work right
              now?
            </p>
            <p style={S.pOKW2hint}>
